...
const Login: React.FC<LoginProps> = ({
  ...
  return (
    <div className="h-full flex items-center justify-center gradient-brand p-6" style={{ background: `linear-gradient(135deg, ${globalConfig.theme?.primaryColor || '#1e3a8a'} 0%, ${globalConfig.theme?.secondaryColor || '#3b82f6'} 100%)` }}>
      <div className="bg-white rounded-3xl shadow-2xl p-8 w-full max-w-md animate-slide-up relative">
        ...
        <div className="text-center mb-6 pt-4">
          <div className="w-16 h-16 rounded-2xl mx-auto flex items-center justify-center shadow-lg mb-4" style={{ backgroundColor: globalConfig.theme?.primaryColor || '#1e3a8a' }}>
        ...
        <form onSubmit={handleLogin} className="space-y-3">
          ...
          <button type="submit" className="w-full text-white font-black py-4 rounded-xl transition-all shadow-lg uppercase tracking-widest mt-2 text-xs" style={{ backgroundColor: globalConfig.theme?.primaryColor || '#1e3a8a' }}>
            Ingresar
          </button>
        </form>
      </div>
